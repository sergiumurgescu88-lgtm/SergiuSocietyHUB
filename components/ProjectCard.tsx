...
export default function ProjectCard({ project, index, onOpenPreview }: ProjectCardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative bg-[#111427] border border-white/10 rounded-2xl p-6 overflow-hidden transition-colors duration-300 hover:shadow-2xl hover:shadow-black/50 cursor-pointer h-full flex flex-col",
    ...
    >
      ...
      <span className="block text-3xl mb-4">{project.emoji}</span>
      <h3 className="font-display font-bold text-white text-lg mb-2 leading-tight">
        {project.name}
      </h3>
      <p className="text-[#6b7299] text-sm leading-relaxed mb-6">
        {project.description}
      </p>

      <div className="flex items-center justify-between gap-4 mt-auto">
        {project.status === 'under-construction' ? (
          <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold bg-white/5 text-[#6b7299] border border-white/10 cursor-not-allowed">
            {t.underConstruction}
          </div>
        ) : (
          <a
            ...
            className={cn(
              "inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold transition-all hover:opacity-90 active:scale-95 shadow-lg",
        ...
        {project.status === 'live' ? (
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 uppercase tracking-wider">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
            {t.live}
          </span>
        ) : project.status === 'demo' ? (
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20 uppercase tracking-wider">
            <span className="w-1.5 h-1.5 rounded-full bg-amber-400" />
            DEMO
          </span>
        ) : (
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold bg-white/5 text-[#6b7299] border border-white/10 uppercase tracking-wider">
            OFFLINE
          </span>
        )}
      </div>
